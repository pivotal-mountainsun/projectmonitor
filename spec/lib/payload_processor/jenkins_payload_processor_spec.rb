...
describe JenkinsPayloadProcessor do
  ...
  let(:payload) { JenkinsAtomExample.new(atom).read }
  ...
  describe "project status" do
    ...
    context "when building" do
      it "remains green when existing status is green" do
        payload = JenkinsAtomExample.new("success.atom").read
        JenkinsPayloadProcessor.new(project,payload).perform
        statuses = project.statuses
        payload = BuildingStatusExample.new("jenkins_cimonitor_building.atom").read
        JenkinsPayloadProcessor.new(project,payload).perform
      ...
      it "remains red when existing status is red" do
        payload = JenkinsAtomExample.new("failure.atom").read
        JenkinsPayloadProcessor.new(project,payload).perform
        statuses = project.statuses
        payload = BuildingStatusExample.new("jenkins_cimonitor_building.atom").read
        JenkinsPayloadProcessor.new(project,payload).perform
  ...
  describe "building status" do
    let(:payload) { BuildingStatusExample.new(atom).read }
    context "when building" do
